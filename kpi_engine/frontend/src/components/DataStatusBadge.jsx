...
export default function DataStatusBadge() {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      await fetchStatus();
      setFeedbackMsg({ type: 'success', text: data.message || 'Update complete' });
      setTimeout(() => setFeedbackMsg(null), 4000);
    } catch (e) {
      setFeedbackMsg({ type: 'error', text: 'Update error: ' + (e.message || 'Failed') });
      setTimeout(() => setFeedbackMsg(null), 4000);
    } finally {
      setUpdating(false);
    }
        ...
        <div className="absolute top-full right-0 mt-2 w-80 bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl z-50 p-4 space-y-3">
          ...
          {feedbackMsg && (
            <div className={`p-2 rounded-lg text-[10px] font-medium ${
              feedbackMsg.type === 'success' ? 'bg-emerald-950/80 text-emerald-300 border border-emerald-800' : 'bg-rose-950/80 text-rose-300 border border-rose-800'
            }`}>
              {feedbackMsg.text}
            </div>
          )}
