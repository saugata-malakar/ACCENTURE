...
import { getActionOutcomes, checkActionOutcomes } from '../api/client';
...
export default function ActionOutcomesPage() {
  ...
  const fetchData = async () => {
    try {
      setError(null);
      const res = await getActionOutcomes();
      setData(res);
    } catch (err) {
      setError(err.message || 'Failed to load action outcomes');
    } finally {
      setLoading(false);
    }
  };
  ...
  const triggerCheck = async () => {
    setChecking(true);
    try {
      await checkActionOutcomes();
      await fetchData();
    } catch (err) {
      setError(err.message || 'Failed to check action outcomes');
    } finally {
      setChecking(false);
    }
  };
